...
public class TemplateEngineUtils {
    ...
    public static Dict buildContext(GenTable genTable) {
        ...
        if (ObjectUtil.isNotNull(paramsObj)) {
            String popupComponent = Optional
                ...
                .orElse("modal");
            context.put("popupComponent", popupComponent);
            context.put("PopupComponent", StringUtils.capitalize(popupComponent));
        } else {
            context.put("popupComponent", "modal");
            context.put("PopupComponent", "Modal");
        }
    ...
    public static List<PathNamedTemplate> getTemplateList(String tplCategory, String dsName, String frontendType) {
        ...
        // bell
        if (frontendType.equals(GenConstants.FRONTEND_TYPE_BELL)) {
            templates.add(getTemplate(getFrontendPopTemplatePath(frontendType)));
            templates.add(getTemplate(getFrontendSearchTemplatePath(frontendType)));
            templates.add(getTemplate(getFrontendDataTemplatePath(frontendType)));
        }
        return templates;
    }
    ...
    public static String getFileName(String template, GenTable genTable) {
        ...
        if (template.contains("domain.java.")) {
            ...
            // bell
            if (genTable.getFrontendType().equals(GenConstants.FRONTEND_TYPE_BELL)) {
                fileName = StringUtils.format("{}/api/{}/{}/model.d.ts", frontendPath, moduleName, businessName);
            } else {
        ...
        // bell
        else if (isFrontendPageTemplate(template, GenConstants.FRONTEND_INDEX_DATA_TEMPLATE_PREFIX)) {
            fileName = StringUtils.format("{}/{}/{}/{}/data.{}", frontendPath, frontendPagePath, moduleName, businessName,
                getFrontendPageExtension(template, GenConstants.FRONTEND_INDEX_DATA_TEMPLATE_PREFIX));
        } else if (isFrontendPageTemplate(template, GenConstants.FRONTEND_INDEX_SEARCH_TEMPLATE_PREFIX)) {
            fileName = StringUtils.format("{}/{}/{}/{}/{}search.{}", frontendPath, frontendPagePath, moduleName, businessName, businessName + "-",
                getFrontendPageExtension(template, GenConstants.FRONTEND_INDEX_SEARCH_TEMPLATE_PREFIX));
        } else if (isFrontendPageTemplate(template, GenConstants.FRONTEND_INDEX_POPUP_TEMPLATE_PREFIX)) {
            fileName = StringUtils.format("{}/{}/{}/{}/{}-{}.{}", frontendPath, frontendPagePath, moduleName, businessName, businessName, popupComponent,
                getFrontendPageExtension(template, GenConstants.FRONTEND_INDEX_POPUP_TEMPLATE_PREFIX));
        }
    ...
    private static String getFrontendPopTemplatePath(String frontendType) {
        return getFrontendPageTemplatePath(frontendType, GenConstants.FRONTEND_INDEX_POPUP_TEMPLATE_PREFIX);
    }

    private static String getFrontendSearchTemplatePath(String frontendType) {
        return getFrontendPageTemplatePath(frontendType, GenConstants.FRONTEND_INDEX_SEARCH_TEMPLATE_PREFIX);
    }

    private static String getFrontendDataTemplatePath(String frontendType) {
        return getFrontendPageTemplatePath(frontendType, GenConstants.FRONTEND_INDEX_DATA_TEMPLATE_PREFIX);
    }
